-- El cupo presencial baja de 250 a 225 personas.
-- Las inscripciones ya confirmadas no se tocan: solo cambia el punto a partir
-- del cual una nueva inscripción presencial pasa a lista de espera.
...
declare v_confirmed integer; v_status text; v_existing record; v_capacity constant integer := 225;
begin
  ...
  if p_modality='presencial' and not p_waitlist and (v_existing.id is null or v_existing.modality <> 'presencial' or v_existing.status <> 'confirmed') and v_confirmed >= v_capacity then
  ...
  select count(*) into v_confirmed from public.encuentro_psicologico_registrations where modality='presencial' and status='confirmed';
  return jsonb_build_object('status',v_status,'available',greatest(0,v_capacity-v_confirmed),'changed_existing',v_existing.id is not null);
end; $$;
...
-- El cambio de presencial a virtual solo se ofrece con la sala llena, así que
-- también se mide contra el cupo nuevo.
...
declare
  ...
  v_capacity constant integer := 225;
begin
  perform pg_advisory_xact_lock(hashtext('encuentro_psicologico_capacity'));
  ...
  if v_confirmed < v_capacity then
    return jsonb_build_object('status', 'not_available', 'available', greatest(0, v_capacity - v_confirmed));
  end if;
  ...
  if v_registration.id is null then
    return jsonb_build_object('status', 'not_found', 'available', greatest(0, v_capacity - v_confirmed));
  end if;
