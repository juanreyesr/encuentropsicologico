...
export function buildCertificateHtml({
  ...
  const nameClass = name.length > 58 ? " xxlong" : name.length > 44 ? " xlong" : name.length > 32 ? " long" : "";
...
.name.long{font-size:3.3cqw}.name.xlong{font-size:2.85cqw}.name.xxlong{font-size:2.4cqw}
...
@media print{html,body{width:11in;height:8.5in;background:#fff}.signature .seal{height:auto;-webkit-print-color-adjust:exact;print-color-adjust:exact}.signature .seal-right{width:1.45in}.signature .seal-left{width:.95in}.diploma{width:11in;height:8.5in;min-height:0;padding:.22in;background:#fff}.frame{width:100%;height:100%;aspect-ratio:auto;padding:.36in .58in;outline-offset:-.17in}.brand img{width:1.28in;height:1.28in}.brand p{font-size:10px}.brand span{font-size:10px}.event{font-size:16px}.title{font-size:38px}.title.long{font-size:32px}.title.xlong{font-size:27px}.lead{font-size:14px;margin-top:.15in}.lead.long{font-size:13px}.lead.xlong{font-size:12px}.name{font-size:34px;margin:.11in auto}.name.long{font-size:30px}.name.xlong{font-size:26px}.name.xxlong{font-size:22px}.meta{font-size:12px;margin-top:.1in}.certificate-footer{margin-bottom:.12in}.signatures{margin:.12in auto .1in}.signature{min-height:.82in}.signature img{height:.52in}.signature b{font-size:12px}.signature small{font-size:10px}.logos{min-height:.5in;gap:.18in}.logos img{width:1.48in;height:.5in}}
</style></head><body><main class="diploma"><section class="frame"><header class="brand"><img src="/logo-duelo-arbol-morado.png" alt="Logo del Encuentro Clínico de Psicología" /><div><p>ENCUENTRO CLÍNICO DE PSICOLOGÍA</p><span>Cuando el Duelo se Detiene</span></div></header><section class="certificate-core"><p class="event">${escapeHtml(template.event_name)}</p><h1 class="title${titleClass}">${escapeHtml(title)}</h1><p class="lead${bodyClass}">Se otorga el presente diploma a</p><h2 class="name${nameClass}">${escapeHtml(name)}</h2><p class="lead${bodyClass}">${escapeHtml(body)}</p><p class="meta">${escapeHtml(template.event_date)} · ${escapeHtml(template.event_place)}</p><p class="number">No. ${escapeHtml(certificateNumber)}</p></section><footer class="certificate-footer"><div class="signatures" style="grid-template-columns:repeat(${signatures.length},minmax(0,1fr))">${signatureHtml}</div>${logosHtml}</footer></section></main>${autoPrint ? "<script>window.addEventListener('load',()=>window.print())</script>" : ""}</body></html>`;
}
