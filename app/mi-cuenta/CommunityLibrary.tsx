...
type Resource = { id: number; title: string; description: string | null; source_author: string | null; original_filename: string; mime_type: string; size_bytes: number; status: string; moderation_reason: string | null; created_at: string; download_count?: number };
type Group = Category & { resources: Resource[] };
type LibraryData = { availableCategories: Category[]; groups: Group[]; myResources: Resource[]; rewards: { stars: number; contributions: number; usefulDownloads: number }; pendingContributions: number };
...
export default function CommunityLibrary() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("¡Has ganado una estrella por apoyar a la comunidad! (pendiente de aprobación). Tu recurso aparecerá en la biblioteca cuando sea revisado.");
    await loadLibrary();
    router.refresh();
  }
  ...
  return <section className="community-library" aria-labelledby="community-library-title">
    ...
    {data && <div className="community-reward-summary"><span className="reward-star"><StarIcon filled /></span><div><b>{data.rewards.stars} {data.rewards.stars === 1 ? "estrella" : "estrellas"}</b><small>{data.rewards.contributions} aportes aprobados · {data.rewards.usefulDownloads} descargas útiles de otros miembros</small>{data.pendingContributions > 0 && <small className="community-pending-reward">+{data.pendingContributions} pendiente{data.pendingContributions === 1 ? "" : "s"} de aprobación</small>}</div></div>}
    {message && <p className="community-success" role="status">{message}</p>}
    {error && !modal && <p className="community-error" role="alert">{error}</p>}

    {loading ? <div className="community-loading" aria-live="polite">Preparando la biblioteca…</div> : data?.groups.length ? <><p className="community-expand-hint">Presiona cada categoría para ver su contenido.</p><div className="community-groups">{data.groups.map(group => <details className="community-category" key={group.id}><summary><div><span>{String(group.resources.length).padStart(2, "0")}</span><h3>{group.name}</h3></div><p>{group.resources.length} {group.resources.length === 1 ? "recurso" : "recursos"}</p><b aria-hidden="true">+</b></summary><div className="community-resource-grid">{group.resources.map(resource => <article key={resource.id}><div className="community-file-icon"><FileIcon /></div><span>{resource.original_filename.split(".").pop()?.toUpperCase()} · {Math.max(1, Math.round(resource.size_bytes / 1024))} KB</span><h4>{resource.title}</h4><p>{resource.description || "Recurso compartido para la comunidad profesional."}</p>{resource.source_author && <small>Autor o fuente: {resource.source_author}</small>}<footer><div><b>Recurso de la comunidad</b><small>{resource.download_count ?? 0} descargas</small></div><a href={`/api/community-resources/${resource.id}/download`} aria-label={`Descargar ${resource.title}`}>Descargar <span>↓</span></a></footer></article>)}</div></details>)}</div></> : <div className="community-empty"><span><FileIcon /></span><h3>La biblioteca está lista para recibir el primer aporte.</h3><p>Comparte un material útil y ayúdanos a construir este espacio entre profesionales y estudiantes.</p><button type="button" className="secondary" onClick={() => setModal("declaration")}>Compartir el primer recurso</button></div>}
