...
export async function POST() {
  ...
  const [settingsResponse, registrationsResponse] = await Promise.all([
    ...
    supabaseServerFetch("encuentro_psicologico_registrations?select=id,user_id,attendee_type&status=eq.confirmed&user_id=not.is.null"),
  ]);
  ...
  if (!rows.length) return Response.json({ ok: true, generated: 0, message: "No hay participantes confirmados todavía." });
