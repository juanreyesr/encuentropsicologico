...
import { programTimeLabel, youtubeEmbedUrl, youtubeVideoId } from "../../lib/event";
...
const emptyDashboard: DashboardData = { daysRemaining: 0, metrics: { total: 0, presencial: 0, virtual: 0, waitlist: 0, available: 225, capacity: 225, students: 0, professionals: 0, networkOptIns: 0, sharedDirectory: 0, guatemala: 0, speakers: 0, organizers: 0 }, recent: [], problems: { open: 0, recent: [] }, professionalNetwork: { optIns: [], shared: [] }, statistics: { departments: [], genders: [], professions: [], countries: [], modalities: [], profiles: [] } };
...
export default function AdminDashboard({ userName }: { userName: string }) {
  ...
  const liveVideoId = youtubeVideoId(content.liveUrl);
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-main">
      ...
      {active === "Transmisión" && <div className="admin-content"><div className="panel transmission-editor">
        <p className="admin-kicker">CONTROL DE ACCESO</p>
        <h2>Transmisión en vivo</h2>
        <p>Pega el enlace de YouTube y actívala. Solo las personas con cuenta del evento podrán verla, dentro de esta plataforma.</p>
        <div className="transmission-fields">
          <label>Nombre de la transmisión<input value={content.liveTitle} placeholder="Ej. Jornada Clínica en vivo" onChange={event => setContent({ ...content, liveTitle: event.target.value })} /><small>Aparece sobre el video en la sala y en el cuadro de los participantes.</small></label>
          <label>Enlace de YouTube<input type="url" inputMode="url" spellCheck={false} value={content.liveUrl} placeholder="https://www.youtube.com/watch?v=..." onChange={event => setContent({ ...content, liveUrl: event.target.value.trim() })} /><small>Admite youtube.com/watch, youtu.be, /live y /embed. Pega el enlace completo tal como lo copias de YouTube.</small></label>
          {content.liveUrl && (liveVideoId
            ? <p className="transmission-check ok">Enlace reconocido · video <code>{liveVideoId}</code></p>
            : <p className="transmission-check bad">No reconocemos ese enlace de YouTube. Revisa que sea la dirección completa del video o de la transmisión.</p>)}
        </div>
        {liveVideoId && <div className="transmission-preview"><span>VISTA PREVIA</span><div className="live-box-frame"><iframe src={youtubeEmbedUrl(liveVideoId)} title="Vista previa de la transmisión" allow="accelerometer; clipboard-write; encrypted-media; picture-in-picture" allowFullScreen /></div><small>Así se verá el cuadro dentro de la cuenta de cada participante y en la portada.</small></div>}
        <label className="live-toggle large"><div><b>Mostrar transmisión</b><small>{content.live ? "Visible para participantes registrados" : "Aún no disponible"}</small></div><input type="checkbox" checked={content.live} onChange={event => setContent({ ...content, live: event.target.checked })} /><i /></label>
        <p className="admin-note">Recuerda pulsar <b>Guardar cambios</b> arriba para aplicar el enlace y el estado de la transmisión.</p>
      </div></div>}
