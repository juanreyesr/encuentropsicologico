...
const sections = ["Resumen", "Contenido", "Ponentes", "Problemas", "Transmisión"];
...
export default function AdminDashboard({ userName }: { userName: string }) {
  ...
  return <main className="admin-shell">
    <aside className="admin-sidebar">
      ...
      <nav>{sections.map((item, index) => <button key={item} className={active === item ? "active" : ""} onClick={() => setActive(item)}><span>{navIcons[index]}</span>{item}{item === "Ponentes" && speakers.length > 0 && <i>{speakers.length}</i>}{item === "Problemas" && dashboard.problems.open > 0 && <i className="alert-dot">{dashboard.problems.open}</i>}</button>)}</nav>
    ...
    <section className="admin-main">
      <header><div><p>Panel de administración</p><h1>{active}</h1></div><div className="admin-header-actions"><Link href="/" target="_blank">Ver sitio ↗</Link>{["Contenido", "Ponentes", "Transmisión"].includes(active) && <button className="admin-save" disabled={saving} onClick={active === "Ponentes" ? saveSpeaker : saveContent}>{saving ? "Guardando…" : "Guardar cambios"}</button>}</div></header>
