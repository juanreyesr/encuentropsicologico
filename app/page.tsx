    ...
    <main>
      ...
      <section id="inicio" className="hero">
        ...
        <div className="hero-content" id="contenido">
          ...
          <p className="hero-copy preserve-newlines">{siteContent.description}</p>
          ...
          <div className="hero-meta"><span><b>01</b> jornada</span><span><b>06</b> ponentes</span><span><b>3.5</b> horas</span><span className="hero-directions-group"><a className="hero-directions" href="https://maps.app.goo.gl/BTYVt9Fs7JeiQ1vw5" target="_blank" rel="noreferrer" aria-label="Cómo llegar al evento con Google Maps"><i aria-hidden="true">⌖</i><span>Cómo llegar<small>Google Maps</small></span><em aria-hidden="true">↗</em></a><a className="hero-directions waze" href="https://waze.com/ul/h9fx7m7sx9" target="_blank" rel="noreferrer" aria-label="Cómo llegar al evento con Waze"><i aria-hidden="true"><svg viewBox="0 0 24 24" role="presentation"><path d="M12 2.6c5 0 8.6 3.3 8.6 7.6 0 1.4-.4 2.7-1.1 3.8a3.7 3.7 0 0 0-6.3 2.4c-.4 0-.8.1-1.2.1-1.6 0-3.1-.3-4.4-.9a3.7 3.7 0 0 0-4-2.6A7 7 0 0 1 3.4 10c0-4.3 3.6-7.4 8.6-7.4Z" fill="currentColor"/><circle cx="9.3" cy="9.2" r="1.25" fill="#0b1030"/><circle cx="14.7" cy="9.2" r="1.25" fill="#0b1030"/><path d="M9 12.4a3.4 3.4 0 0 0 6 0" stroke="#0b1030" strokeWidth="1.3" fill="none" strokeLinecap="round"/><circle cx="6.2" cy="18.6" r="1.9" fill="currentColor"/><circle cx="16.4" cy="18.6" r="1.9" fill="currentColor"/></svg></i><span>Cómo llegar<small>Waze</small></span><em aria-hidden="true">↗</em></a></span></div>
      ...
      <section id="agenda" className="agenda-section section-pad">
        ...
        <div className="agenda-list">{visibleAgenda.map((item, i) => <article key={item.id}><time>{programTimeLabel(item)}</time><span className={`tag tag-${i % 3}`}>{item.type}</span><div><h3 className="preserve-newlines">{item.title}</h3><p className="preserve-newlines">{item.description}</p></div><button aria-label={`Ver detalles de ${item.title}`} onClick={() => openProgramDetail(item)}>+</button></article>)}</div>
      ...
      <section className="partners section-pad"><p className="section-kicker">HACEN POSIBLE ESTE ENCUENTRO</p><h2>Aliados por la salud mental.</h2>{sponsors.length ? <div className="sponsor-public-grid">{sponsors.map(sponsor => <article key={sponsor.id}>{sponsor.logo_url ? <img src={sponsor.logo_url} alt={`Logo de ${sponsor.name}`} /> : <div className="sponsor-monogram">{sponsor.name.slice(0,1)}</div>}<div className="sponsor-public-copy"><b className="preserve-newlines">{sponsor.name}</b>{sponsor.description && <p className="preserve-newlines">{sponsor.description}</p>}{sponsor.website && <a href={sponsor.website.startsWith("http") ? sponsor.website : `https://${sponsor.website}`} target="_blank" rel="noreferrer">Conocer aliado <span aria-hidden="true">↗</span></a>}</div></article>)}</div> : <div className="public-empty"><span>CONVOCATORIA ABIERTA</span><h3>Patrocinadores por anunciar.</h3><p>Las organizaciones confirmadas aparecerán aquí.</p></div>}<a href="mailto:lic.juanreyesr@gmail.com">Quiero ser patrocinador <span>→</span></a></section>
